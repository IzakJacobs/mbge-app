...
if ($action === 'login') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($step === 'credentials' && isset($_POST['username'], $_POST['password'])) {
            ...
            if ($lockCheck['locked']) {
                ...
                $stmt = db()->prepare(
                    "SELECT id, name, username, pin, phone, email, device_token, password_changed_at
                ...
                if (!$sec || !password_verify($pass, $sec['pin'])) {
                    ...
                    if (empty($sec['device_token'])) {
                        ...
                        // New device — real emailed OTP (mirrors admin.php).
                        // Previously this compared against the last 6 digits
                        // of the officer's own phone number, which isn't a
                        // secret and isn't one-time.
                        $secEmail = trim($sec['email'] ?? '');
                        if ($secEmail === '') {
                            $error = 'No email address is on file for this security officer '
                                   . 'account, so a one-time code cannot be sent. Please ask '
                                   . 'an administrator to add your email address before '
                                   . 'logging in from a new device.';
                        } else {
                            $_SESSION['sec_login_step']    = 'otp';
                            $_SESSION['sec_pending_id']    = $sec['id'];
                            $_SESSION['sec_pending_email'] = $secEmail;
                            require_once __DIR__ . '/twilio_helper.php';
                            generateEmailOtp($secEmail);
                            header('Location: security.php?action=login'); exit;
                        }
                    }
                }
        ...
        elseif ($step === 'otp' && isset($_POST['otp'])) {
            ...
            $email = $_SESSION['sec_pending_email'] ?? '';
            require_once __DIR__ . '/twilio_helper.php';
    ...
    $maskedEmail = '';
    if ($step === 'otp' && !empty($_SESSION['sec_pending_email'])) {
        $parts = explode('@', $_SESSION['sec_pending_email']);
    ...
    <div class="login-wrap">
      <div class="login-card">
        ...
        <div class="alert alert-info" style="font-size:.88rem;">
          ...
          We emailed a <strong>6-digit code</strong> to <strong><?= htmlspecialchars($maskedEmail) ?></strong>.
...
function securityGrantAccess(array $sec): void {
    ...
    unset($_SESSION['sec_login_step'], $_SESSION['sec_pending_id'], $_SESSION['sec_pending_email']);
