...
// Emoji matches each role's login-logo (admin.php ⚙️, security.php 🛡️,
// guard.php 🔐, resident.php 🏠) so the same visual identity carries
// through from the login screen into every page inside the app, not
// just the login screen — previously this was a single hardcoded 🏌️
// (golf) emoji regardless of role, which didn't differentiate the
// portals from each other once a user was logged in.
function renderHeader(string $title, string $logoutUrl = 'logout.php'): void {
    ...
    echo '<h1>' . $emoji . ' ' . htmlspecialchars($title) . '</h1>';
